...
class PositionalEncoding(nn.Module):
    def __init__(self, d_model:int, seq_len:int, dropout:float):
        ...
        # create a matrix shape of (seq_len, d_model)

        pe = torch.zeros(seq_len, d_model)

        # create a vector opf seqlen

        position = torch.arange(0,seq_len, dtype=torch.float()).unsqueeze(1)
        div_term = torch.exp(torch.arrange(0,d_model,2).float() * (-math.log(10000.0) / d_model))

        # apply sin to even position vector and cos to odd position vector

        pe[:,0::2] = torch.sin(position * div_term)
        pe[:,1::2] = torch.cos(position * div_term)

        pe = pe.unsqueeze(0) # add a tenser
...
class FeedForwardBlock(nn.Module):
    def __init__(self, d_model :int, d_ff : int, dropout: float) -> None:
        super().__init__()
        self.linear_1 = nn.Linear(d_model, d_ff) # W1 anmd b1
        self.dropout = nn.Dropout(dropout)
...
class multiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask):
        query = self.w_q(q)  # batch, seq_len,, demodel --> batch, seq_len, d_model
        key = self.w_k(k) # batch, seq_len,, demodel --> batch, seq_len, d_model
        value = self.w_v(v) # batch, seq_len,, demodel --> batch, seq_len, d_model
 
        # batch,seq_len,dmodel --> batch, seq_len, h, d_k --> batch, h, seq_len, d_k
...
def build_transformer(src_vocab_size: int, tgt_vocab_size : int, src_seq_len:int, tgt_seq_len: int, d_model: int=512, N : int=6, h : int=8, dropout:float = 0.1, d_ff:int = 2048) -> Transformer:
    ...
    for _ in range(N):
        ...
        decoder_cross_attentionblock = multiHeadAttention(d_model, h, dropout)
        feed_forward_block = FeedForwardBlock(d_model, d_ff, dropout)
        decoder_block = DecoderBlock(decoder_self_attention, decoder_cross_attentionblock, feed_forward_block, dropout)
    ...
    # Initialize athe parammeter (Xavier) fast training
